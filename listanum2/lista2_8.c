...
fwrite(vet,sizeof(CADASTRO),1, arq);

fclose(arq);

fopen("cadastro.bin", "rb");
if (!arq){
    printf("O arquivo não existe");
    exit(1);
}

while(fread(vet, sizeof(CADASTRO), 1, arq)){
    printf("\n\n*****SECAO DE UTILIZACAO***** \n ola %s, o seu endereco eh %s e a sua idade eh %d \n\n", vet->nome, vet->endereco, vet->idade);
}
fclose(arq);
